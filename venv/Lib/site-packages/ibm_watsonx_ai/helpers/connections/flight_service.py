...
class FlightConnection(BaseFlightConnection):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if kwargs.get("data_size_limit"):
            data_size_limit_parameter_deprecated_warning = (
                "The parameters data_size_limit in FlightConnection is deprecated. "
                "Use total_size_limit instead."
            )
            warn(data_size_limit_parameter_deprecated_warning, category=DeprecationWarning)  # fmt: skip
    ...
    def get_endpoints(self) -> Iterable[List["flight.FlightEndpoint"]]:
        ...
        for source_command in self._select_source_command():
            ...
            while attempt < max_retries + 1:
                ...
                sleep_seconds = min(
                    pow(2.0, attempt) * jitter + prolong, self.max_retry_time
                )
    ...
    def read(self) -> Generator[pd.DataFrame, None, None]:
        ...
        except (InvalidSamplingType, WrongFileLocation) as e:
            raise e
    ...
    def create_logical_batch(
        ...
        while True:
            try:
                ...
                self._logger.debug(
                    "LB: Waiting for mini batch to appear in the queue..."
                )
            ...
            if max_rows > 0:
                ...
                self._logger.debug("LB: Yielding logical batch!")
    ...
    def _select_source_command(self, infer_schema: bool = False) -> List[str]:
        ...
        if "bucket" in self.data_location["location"]:
            ...
            command["interaction_properties"].pop(
                "userfs", None
            )  # userfs flag for git based project is set on higher lvl
            if infer_schema:
                command["interaction_properties"]["infer_schema"] = "true"
    ...
    def data_dropna(self, data, label_column):
        ...
        try:
            ...
            warn(f"Error dropping nan values from column {label_column}")
        return data

    @staticmethod
    def _apply_literal_eval(data: pd.DataFrame):
        for i, (col_name, col_type) in enumerate(zip(data.dtypes.index, data.dtypes)):
            if col_type == "object":
                try:
                    ...
                    warn(f"Applying literal_eval to data column '{col_name}' error: {e}")  # fmt: skip
        return data

    @_flight_retry()
    def discovery(self, path: str, details: bool = False):
        ...
        command = {
            "context": "target",
            "path": path,
            "detail": "true" if details else "false",
        ...
        while True:
            action = pa.flight.Action("discovery", json.dumps(command).encode("utf-8"))
            action_response = self.flight_client.do_action(action)
            response_parsed = json.loads(next(action_response).body.to_pybytes())
            new_assets = response_parsed.get("assets", [])
            if (
                not result and not new_assets
